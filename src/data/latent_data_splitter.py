...
class LatentsSplitter:
    ...
    def __init__(self, featname, use_stim, subject, nsd_dir, output_dir):
        ...
        self.nsd_expdesign = scipy.io.loadmat(os.path.join(nsd_dir, 'nsddata', 'experiments', 'nsd', 'nsd_expdesign.mat'))
        self.sharedix = self.nsd_expdesign['sharedix'][0] - 1  # Convert to 0-based indexing

    def load_stimuli(self):
        """Load stimuli indices."""
        try:
            stims_path = os.path.join(self.output_dir, f'fmri_features/{self.subject}', f'{self.subject}_stims_{self.use_stim}.npy')
    ...
    def split_latent_features(self):
        ...
        for idx, s in tqdm(enumerate(stims), total=len(stims), desc="Processing stimuli"):
            ...
            feat_path = os.path.join(self.output_dir, f"latent_features/{self.featname}/{s:06}.npy")
        ...
        splits_path = os.path.join(self.output_dir, f"latent_features/{self.subject}/splits")
...
def main():
    ...
    parser.add_argument("--use_stim", type=str, required=True, choices=['ave', 'each'], help="Stimulus type (ave or each)")
