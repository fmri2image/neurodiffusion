...
class FeatureExtractor(ABC):
    def __init__(self, img_idx, gpu, captions_num, output_dir, subject, nsd_root, packages_path):
        ...
        os.makedirs(os.path.join(output_dir, f'latent_features/extracted_latents/init_latent/{self.subject}'), exist_ok=True)
        os.makedirs(os.path.join(output_dir, f'latent_features/extracted_latents/c_top{captions_num}_captions'), exist_ok=True)
...
class StableDiffusionExtractor(FeatureExtractor):
    def __init__(self, img_idx, gpu, captions_num, output_dir, subject, nsd_root, packages_path):
        super().__init__(img_idx, gpu, captions_num, output_dir, subject, nsd_root, packages_path)

        self.config_path = os.path.join(self.packages_path, "stable-diffusion/configs/stable-diffusion/v1-inference.yaml")
        self.ckpt_path = os.path.join(self.packages_path, "stable-diffusion/models/ldm/stable-diffusion-v1/sd-v1-4.ckpt")
    ...
    def extract_features(self):
        ...
        for img_idx in tqdm(img_indices):
            ...
            if os.path.exists(os.path.join(self.output_dir, f"latent_features/extracted_latents/init_latent/{self.subject}/{img_str}.npy")):
                logger.info(f"Skipping {img_str}, already processed.")
                continue

            logger.info(f"Processing image {img_str}")
            img = self.nsda.read_images(img_idx)
            Image.fromarray(img.astype(np.uint8)).save(os.path.join(self.output_dir, f"original_images/{self.subject}/{img_str}_original.png"))
