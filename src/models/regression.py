...
class RegressionModel(ABC):
    ...
    def __init__(self, target, roi, subject, fmri_dir, feat_dir, models_dir):
        ...
        self.model = None
        self.backend = set_backend("torch_cuda" if 'mlp' in self.__class__.__name__.lower() else "numpy", on_error="warn")
    ...
    def load_data(self):
        ...
        try:
            ...
            for croi in self.roi:
                ...
                X_te.append(np.load(os.path.join(self.fmri_dir, f"{self.subject}_{croi}_betas_ave_te.npy")).astype("float32"))
            X = np.hstack(X)
            X_te = np.hstack(X_te)
    ...
    def predict_and_evaluate(self, X_te, Y_te):
        ...
        scores_path = os.path.join(self.feat_dir, f"scores/{self.subject}_{'_'.join(self.roi)}_scores_{self.target}.npy")
...
class RidgeRegression(RegressionModel):
    ...
    def fit(self, X, y):
        ...
        alphas = [0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1] if self.target in ['init_latent', 'c_top5_captions'] else [10000, 20000, 40000]
...
class KernelRidgeRegression(RegressionModel):
    ...
    def fit(self, X, y):
        ...
        logger.info(f"Fitting KernelRidgeRegression for target {self.target}, ROIs {self.roi}, subject {self.subject} with PCA={self.pca_dim}")
...
def main():
    ...
    parser.add_argument("--roi", type=str, required=True, nargs='+', help="Regions of interest (e.g., early ventral midventral ...)")
    ...
    parser.add_argument("--latent_feat_dir", type=str, default="/mnt/outputs/latent_features", help="Latent Features directory")
    parser.add_argument("--models_dir", type=str, default="/mnt/outputs/saved_models", help="Output directory for models and predictions")
    parser.add_argument("--model", type=str, default="all", choices=['ridge', 'kernelridge', 'mlp', 'all'], help="Model to train (default: all)")
    ...
    for model_class in selected_models:
        ...
        print(f"{model_class.__name__ if hasattr(model_class, '__name__') else 'MLPRegression'} saved to: {model_path}, Mean correlation: {mean_rs:.3f}")
