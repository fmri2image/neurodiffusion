...
class DiffusionDecoder:
    ...
    def __init__(self, imgidx, method, subject, gpu, seed, nsd_dir, output_dir, config_path, ckpt_path):
        ...
        self.nsd_expdesign = scipy.io.loadmat(os.path.join(self.nsd_dir, 'nsddata', 'experiments', 'nsd', 'nsd_expdesign.mat'))
    ...
    def _load_init_latent(self, imgidx):
        ...
        if self.method in ['mlp-regressor']:
            scores_path = os.path.join(
                self.output_dir,
                os.path.join(self.output_dir, f"latent_features/{self.subject}/scores/{self.subject}_early_scores_init_latent.npy")
            )
            ...
            # No PCA
            scores_path = os.path.join(self.output_dir, f"latent_features/{self.subject}/scores/{self.subject}_early_scores_init_latent.npy")  
            scores_latent = np.load(scores_path)
            ...
            with torch.no_grad(), precision_scope("cuda"):
                with self.model.ema_scope():
                    for n in trange(self.n_iter, desc="Sampling"):
                        ...
                        z_enc = self.sampler.stochastic_encode(init_latent, torch.tensor([int(self.strength * self.ddim_steps)] * self.batch_size).to(self.device))
                        samples = self.sampler.decode(z_enc, c, int(self.strength * self.ddim_steps), unconditional_guidance_scale=self.scale, unconditional_conditioning=uc)
...
def main():
    ...
    parser.add_argument("--method", type=str, required=True, choices=['ridge', 'kernel-ridge', 'mlp-regressor'], help="Decoding method")
    ...
    parser.add_argument("--config_path", type=str, default="diffusion_sd1/stable-diffusion/configs/stable-diffusion/v1-inference.yaml", help="Stable Diffusion config path")
    parser.add_argument("--ckpt_path", type=str, default="data/models/stable-diffusion/sd-v1-4.ckpt", help="Stable Diffusion checkpoint path")
